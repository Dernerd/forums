...
Description: Ermöglicht jedem Blog, seine eigenen Foren zu haben - eingebettet in jede Seite oder jeden Beitrag.
Author: WMS N@W
Author URI: https://n3rds.work
Version: 2.0.2
Text Domain: psource_forums
*/
...
?>
<p><strong><?php printf( __( 'Besuche die Registerkarte „Upgrades“, um weitere Foren zu erhalten. Derzeit bist Du auf %s beschränkt', 'psource_forums' ), $forums_max_forums ); ?></strong></p>
<?php
}
...
add_action('admin_print_styles-toplevel_page_psource_forums', 'forums_admin_styles');
add_action('admin_print_scripts-toplevel_page_psource_forums', 'forums_admin_scripts');
...
function forums_plug_init() {
	...
	if ($forums_enable_upgrades == 1) {
		...
		upgrades_register_feature( '68daf8bdc8755fe8f4859024b3054fb8', __( 'Foren', 'psource_forums' ), __( 'Zusätzliche Foren', 'psource_forums' ) );
	...
	if ( defined( 'WPMU_PLUGIN_DIR' ) && file_exists( WPMU_PLUGIN_DIR . '/' . basename( __FILE__ ) ) ) {
		load_muplugin_textdomain( 'psource_forums', WPMU_PLUGIN_DIR .'forums/languages' );
	} else {
		load_plugin_textdomain( 'psource_forums', false, 'forums/languages' );
	}
...
function forums_plug_pages() {
	...
	if ( $forums_enable_upgrades && FORUM_DEMO_FOR_NON_SUPPORTER && function_exists('is_pro_site') && !is_pro_site()) {
		add_menu_page( __( 'Foren', 'psource_forums' ), __( 'Foren', 'psource_forums' ), 'manage_options', 'psource_forums', 'forums_non_supporter_output');
	} else {
		add_menu_page( __( 'Foren', 'psource_forums' ), __( 'Foren', 'psource_forums' ), 'manage_options', 'psource_forums', 'forums_manage_output');
	}
}
...
function forums_options_plug_pages() {
	$page = WP_NETWORK_ADMIN ? 'settings.php' : 'options-general.php';
        ...
        add_submenu_page($page, __( 'Forum Einstellungen', 'psource_forums' ), __( 'Foren', 'psource_forums' ), $perms, 'psource_forum_settings', 'forums_manage_options_output');
}

function forums_non_supporter_output() {
	global $blog_id, $psts;
	?>
	<h3><?php _e('Nur für PRO...', 'psource_forums'); ?></h3>
...
function forums_admin_bar( $menu ) {
	unset( $menu['admin.php?page=psource_forums'] );
	return $menu;
}
		...
		if ($tmp_forum_count > 0){
			...
			if ($_action == 'new_topic'){
				...
				if ($tmp_user_count > 0){
					...
					$tmp_errors = $tmp_errors + 1;
					$tmp_error_msg = __( 'Ungültiges Benutzerkonto...', 'psource_forums' );
				}
				//check for invalid forum ID
				if ($_POST['fid'] != $tmp_fid){
					$tmp_errors = $tmp_errors + 1;
					$tmp_error_msg = __( 'Ungültiges Forum...', 'psource_forums' );
				}
				if ($tmp_errors > 0){
					...
					if ($tmp_tid == ''){
						...
						$content = $content . '<p><center>' . __( 'Thema hinzugefügt!', 'psource_forums' ) . '</center></p>';
				...
				if ($tmp_topic_count > 0){
					if ($_action == 'close_topic'){
						if(current_user_can('manage_options')) {
							...
							$tmp_msg = __( 'Thema geschlossen!', 'psource_forums' );
						} else {
							$tmp_msg = __( 'Erlaubnis verweigert...', 'psource_forums' );
						}
					}
					if ($_action == 'open_topic'){
						if(current_user_can('manage_options')) {
							...
							$tmp_msg = __( 'Thema geöffnet!', 'psource_forums' );
						} else {
							$tmp_msg = __( 'Erlaubnis verweigert...', 'psource_forums' );
						}
					}
					...
					if ($tmp_topic_closed == 1){
						$content = $content . '<h3>' . $tmp_topic_title . ' (' . __( 'Geschlossen', 'psource_forums' ) . ')</h3>';
					} else {
				...
				if ($tmp_topic_count > 0){
					if(current_user_can('manage_options')) {
						...
						$content = $content . '<p><center>' . __( 'Erlaubnis verweigert...', 'psource_forums' ) . '</center></p>';
					}
				} else {
						$content = $content . '<p><center>' . __( 'Erlaubnis verweigert...', 'psource_forums' ) . '</center></p>';
				}
				...
				if ($tmp_topic_count > 0){
					if ( isset($_POST['Cancel']) ) {
						...
						if(current_user_can('manage_options')) {
							...
							if ($tmp_errors > 0){
								...
								$content = $content . '<p><center>' . __( 'Fehler beim Löschen des Themas...', 'psource_forums' ) . '</center></p>';
								$content = $content . '<br />';
								$content .= forums_get_metadata( $tmp_topic_last_updated, $tmp_topic_last_author );
								$content .= forums_get_navigation( $post->ID, (int)$_GET['topic'], $tmp_topic_closed );
								$content = $content . forums_output_topic_nav((int)$_POST['tid']);
								...
								forums_forum_count_posts($tmp_fid);
								forums_forum_count_topics($tmp_fid);
								$content = $content . '<p><center>' . __( 'Thema gelöscht!', 'psource_forums' ) . '</center></p>';
								$content = $content . forums_output_search_form($tmp_fid);
								$content = $content . '<br />';
								$content = $content . forums_output_forum_nav($tmp_fid);
								$content = $content . '<br />';
					...
					$content = $content . '<p><center>' . __( 'Ungültiges Thema!', 'psource_forums' ) . '</center></p>';
				}
				...
				if ( isset($_POST['Cancel']) ) {
					...
					if ($tmp_errors > 0){
						echo '<script type="text/javascript">';
						echo 'window.location="' . add_query_arg( array( 'topic' => (int)$_POST['tid'], 'forum_page' => (int)$_POST['forum_page'], 'msg' => urlencode( __('Fehler beim Löschen des Beitrags', 'psource_forums' ) ) ), get_permalink() ) . '#post-' . (int)$_POST['pid'] . '";';
						echo '</script>';
					} else {
						echo '<script type="text/javascript">';
						echo 'window.location="' . add_query_arg( array( 'topic' => (int)$_POST['tid'], 'msg' => urlencode( __( 'Beitrag gelöscht', 'psource_forums' ) ) ), get_permalink() ) . '";';
						echo '</script>';
					}
				}
				exit();
				...
				if ( isset($_POST['Cancel']) ) {
					...
					if ($_POST['post_content'] == ''){
						...
						if(current_user_can('manage_options')) {
							...
							echo '<script type="text/javascript">';
							echo 'window.location="' . add_query_arg( array( 'topic' => (int)$_POST['tid'], 'forum_page' => (int)$_POST['forum_page'], 'msg' => urlencode( __( 'Beitrag aktualisiert...', 'psource_forums' ) ) ), get_permalink() ) . '";';
							echo '</script>';
							exit();
						} else {
				...
				if ($user_ID == '' || $user_ID == '0'){
					$content = $content . '<p><center>' . sprintf( __( 'Du musst ein registrierter und eingeloggter Benutzer dieses Blogs sein, um in diesem Forum zu posten. Bitte <a href="%s">Anmelden</a> oder %s.', 'psource_forums' ), wp_login_url(),  wp_register('', '', false) ) . '</center></p>';
				} else {
					...
					if ($tmp_topic_closed != '1'){
						if ($_POST['post_content'] == ''){
							...
							echo '<script type="text/javascript">';
							echo 'window.location="' . add_query_arg( array( 'topic' => (int)$_POST['tid'], 'forum_page' => $tmp_total_pages, 'msg' => urlencode( __( 'Beitrag hinzugefügt...', 'psource_forums' ) ) ), get_permalink() ) . '";';
							echo '</script>';
							exit();
						}
					}
			...
			$content = __( 'Ungültiger Forencode', 'psource_forums' );
		}
		//insert/post content
...
function forums_get_navigation( $post_id, $topic_id, $topic_closed ) {
	...
	$content .= '<a href="' . get_permalink() . '">' . __( 'Zurück zum Index', 'psource_forums' ) . '</a> | ';
	if(current_user_can('manage_options')) {
		$content .= '<a href="' . add_query_arg( array('action' => 'delete_topic', 'tid' => $topic_id ), get_permalink() ) . '">' . __( 'Thema löschen', 'psource_forums' ) . '</a> | ';
		if ($topic_closed == 1){
			$content .= '<a href="' . add_query_arg( array('action' => 'open_topic', 'topic' => $topic_id ), get_permalink() ) . '">' . __( 'Thema öffnen', 'psource_forums' ) . '</a>';
		} else {
			$content .= '<a href="' . add_query_arg( array('action' => 'close_topic', 'topic' => $topic_id ), get_permalink() ) . '">' . __( 'Thema schließen', 'psource_forums' ) . '</a>';
		}
	}
	$content .= '</center>';
	$content .= '<hr />';
...
function forums_get_metadata( $last_updated, $last_author ) {
	...
	$content .= '<li>' . __( 'Zuletzt aktualisiert: ', 'psource_forums' ) . date(get_option('date_format', __("j. F Y g:i", 'psource_forums')),$last_updated) . '</li>';
	$content .= '<li>' . __( 'Neueste Antwort von: ', 'psource_forums' ) . forums_author_display_name($last_author) . '</li>';
	$content .= '</ul>';
	return $content;
}
...
function forums_output_search_results($tmp_fid,$tmp_query){
	...
	$content = $content . '<h3>' . __( 'Suchergebnisse', 'psource_forums' ) . '</h3>';
	$content = $content . forums_output_search_form($tmp_fid);
	$content = $content . '<br />';
	$content = $content . '<h3>' . __( 'Beiträge:', 'psource_forums' ) . '</h3>';
	...
	if (count($tmp_results) > 0){
		...
		$content =  $content . '<p><center>' . __( 'Keine Treffer...', 'psource_forums' ) . '</center></p>';
	}
...
function forums_output_search_form($tmp_fid){
	...
	if ($tmp_topic_count > 0){
		...
		$content = $content . '<input type="submit" name="Submit" value="' . __( 'Suche', 'psource_forums' ) . ' &raquo;" />';
...
function forums_output_new_post_separate($tmp_fid,$tmp_tid,$tmp_errors,$tmp_error_msg = ''){
	...
	if ($user_ID == '' || $user_ID == '0'){
		...
		if ($tmp_topic_closed != '1'){
			$content = $content . '<h3>' . __( 'Neues Thema', 'psource_forums' ) . '</h3>';
			if ($tmp_errors > 0){
				if ($tmp_error_msg == ''){
					$tmp_error_msg = __( 'Du musst alle Pflichtfelder ausfüllen...', 'psource_forums' );
				}
			...
			$content = $content . '<th scope="row">' . __( 'Beitrag:', 'psource_forums' ) . '</th>';
			...
			$content = $content . '<br />';
			$content = $content . __( 'Erforderlich', 'psource_forums' ) . '</td>';
			$content = $content . '</tr>';
			$content = $content . '</table>';
			$content = $content . '</fieldset>';
			$content = $content . '<p class="submit">';
			$content = $content . '<input type="submit" name="Submit" value="' . __( 'Beitrag senden &raquo;', 'psource_forums' ) . '" />';
			$content = $content . '</p>';
			$content = $content . '</form>';
		}
	}
...
function forums_output_new_post($tmp_fid,$tmp_tid,$tmp_errors,$tmp_error_msg = '') {
	...
	if ($user_ID == '' || $user_ID == '0'){
		...
		if ($tmp_topic_closed != '1'){
			$content = $content . '<hr />';
			$content = $content . '<br />';
			$content = $content . '<h3>' . __( 'Neue Antwort', 'psource_forums' ) . '</h3>';
			if ($tmp_errors > 0){
				if ($tmp_error_msg == ''){
					$tmp_error_msg = __( 'Du musst alle Pflichtfelder ausfüllen...' );
				}
			...
			$content = $content . '<table width="100%" cellspacing="2" cellpadding="5">';
			$content = $content . '<tr valign="top">';
			$content = $content . '<th scope="row">' . __( 'Antwort:', 'psource_forums' ) . '</th>';
			$content = $content . '<td><textarea name="post_content" id="post_content" style="width: 95%;  border:1px solid lightgrey" rows="5">' . esc_textarea(isset($_POST['post_content'])?$_POST['post_content']:'') . '</textarea>';
			$content = $content . '<br />';
			$content = $content . __( 'Erforderlich', 'psource_forums' ) . '</td>';
			$content = $content . '</tr>';
			$content = $content . '</table>';
			$content = $content . '</fieldset>';
			$content = $content . '<p class="submit">';
			$content = $content . '<input type="submit" style="padding: 5px; margin:5px; border: 1px solid lightgrey; float: right" name="Submit" value="' . __( 'Antwort senden', 'psource_forums' ) . ' &raquo;" />';
			$content = $content . '</p>';
			$content = $content . '</form>';
		}
	}
...
function forums_output_edit_post($tmp_pid,$tmp_fid,$tmp_tid,$tmp_errors,$tmp_error_msg = '',$tmp_page) {
	...
	$tmp_post_content = $wpdb->get_var($wpdb->prepare("SELECT post_content FROM " . $db_prefix . "forums_posts WHERE post_topic_ID = %d AND post_ID = %d", $_GET['tid'], $_GET['pid']));
		...
		$content .= '<h3>' . __('Beitrag bearbeiten', 'psource_forums') . '</h3>';
		if ($tmp_errors > 0) {
			if ($tmp_error_msg == '') {
				$tmp_error_msg = __('Du musst alle Pflichtfelder ausfüllen...', 'psource_forums');
			}
			$content .= '<p><center>' . $tmp_error_msg . '</center></p>';
		}
		$content .= '<form name="edit_post" method="POST" action="' . add_query_arg(array('action' => 'edit_post_process'), get_permalink()) . '">';
		$content .= '<input type="hidden" name="uid" value="' . $user_ID . '" />';
		if ($tmp_errors > 0) {
			$content .= '<input type="hidden" name="pid" value="' . (int) $_POST['pid'] . '" />';
		} else {
			$content .= '<input type="hidden" name="pid" value="' . (int) $_GET['pid'] . '" />';
		}
		$content .= '<input type="hidden" name="tid" value="' . (int) $tmp_tid . '" />';
		$content .= '<input type="hidden" name="fid" value="' . (int) $tmp_fid . '" />';
		$content .= '<input type="hidden" name="forum_page" value="' . (int) $tmp_page . '" />';
		$content .= '<fieldset style="border:none;">';
		$content .= '<table width="100%" cellspacing="2" cellpadding="5">';
		$content .= '<tr valign="top">';
		$content .= '<th scope="row">' . __('Beitrag:', 'psource_forums') . '</th>';
		if ($tmp_errors > 0) {
			$content .= '<td><textarea name="post_content" id="post_content" style="width: 95%" rows="5">' . esc_textarea($_POST['post_content']) . '</textarea>';
} else {
	...
	$content = $content . '</fieldset>';
	$content = $content . '<p class="submit">';
	$content = $content . '<input type="submit" id="forumbutton" name="Submit" value="' . __( 'Aktualisieren &raquo;', 'psource_forums' ) . '" />';
	$content = $content . '<input type="submit" id="forumbutton" name="Cancel" value="' . __( 'Abbrechen &raquo;', 'psource_forums' ) . '" />';
	$content = $content . '</p>';
	$content = $content . '</form>';
	return $content;
}
...
function forums_output_delete_post($tmp_pid,$tmp_tid,$tmp_page) {
	...
	if ($user_ID == '' || $user_ID == '0'){
		$content = $content . '<h3>' . __( 'Beitrag löschen', 'psource_forums' ) . '</h3>';
		$content = $content . '<p><center>' . __( 'Du musst eingeloggt sein...', 'psource_forums' ) . '</center></p>';
	} else {
		$content = $content . '<h3>' . __( 'Beitrag löschen', 'psource_forums' ) . '</h3>';
		$content = $content . '<br />';
		$content = $content . '<p>' . __( 'Möchtest Du diesen Beitrag wirklich löschen?', 'psource_forums' ) . '</p>';
		...
		$content = $content . '<p class="submit">';
		$content = $content . '<input type="submit" name="Submit" value="' . __( 'Löschen &raquo;', 'psource_forums' ) . '" />';
		$content = $content . '<input type="submit" name="Cancel" value="' . __( 'Abbrechen &raquo;', 'psource_forums' ) . '" />';
		$content = $content . '</p>';
		$content = $content . '</form>';
	}
	return $content;
...
function forums_output_delete_topic($tmp_fid,$tmp_tid) {
	...
	if ($user_ID == '' || $user_ID == '0'){
		$content = $content . '<h3>' . __( 'Thema löschen', 'psource_forums' ) . '</h3>';
		$content = $content . '<p><center>' . __( 'Du musst eingeloggt sein...', 'psource_forums' ) . '</center></p>';
	} else {
		$content = $content . '<h3>' . __( 'Thema löschen', 'psource_forums' ) . '</h3>';
		$content = $content . '<br />';
		$content = $content . '<p>' . __( 'Möchtest Du dieses Thema wirklich löschen?', 'psource_forums' ) . '</p>';
...
function forums_output_topic_nav($tmp_tid){
	...
	if ($tmp_current_page == 1){
		...
		$content = $content . '<td width="25%" style="text-align:left"><a href="'. add_query_arg( array( 'topic' => (int)$_GET['topic'], 'forum_page' => $tmp_previus_page ), get_permalink() ) . '">' . __(' &laquo; Vorherige') . '</a></td>';
	}
	$content = $content . '<td><center>' . sprintf( __( 'Zeige %1s > %2s von %3s Beiträgen', 'psource_forums' ), $tmp_showing_low, $tmp_showing_high, $tmp_post_count ) . '</center></td>';
	if ($tmp_current_page == $tmp_total_pages){
		//last page
		$content = $content . '<td width="25%" style="text-align:right"></td>';
	} else {
		$tmp_next_page = $tmp_current_page + 1;
		$content = $content . '<td width="25%" style="text-align:right"><a href="'. add_query_arg( array( 'topic' => (int)$_GET['topic'], 'forum_page' => $tmp_next_page ), get_permalink() ) . '">' . __( 'Nächste  &raquo;', 'psource_forums' ) . '</a></td>';
	}
	$content = $content . '</tr>';
	$content = $content . '</table>';
	return $content;
...
function forums_output_forum_nav($tmp_fid){
	...
	if ($tmp_topic_count > 0){
		...
		if ($tmp_current_page == 1){
			...
			$tmp_previus_page = $tmp_current_page - 1;
			$content = $content . '<td width="25%" style="text-align:left"><a href="'. add_query_arg( array( 'forum_page' => $tmp_previus_page ), get_permalink() ) . '">&laquo; ' . __( 'Vorherige', 'psource_forums' ) . '</a></td>';
		}
		$content = $content . '<td ><center>' . sprintf( __( 'Zeige %1s > %2s von %3s Themen', 'psource_forums' ), $tmp_showing_low, $tmp_showing_high, $tmp_topic_count ) . '</center></td>';
		if ($tmp_current_page == $tmp_total_pages){
			//last page
			$content = $content . '<td width="25%" style="text-align:right"></td>';
		} else {
			$tmp_next_page = $tmp_current_page + 1;
			$content = $content . '<td width="25%" style="text-align:right"><a href="'. add_query_arg( array( 'forum_page' => $tmp_next_page ), get_permalink() ) . '">' . __( 'Nächste', 'psource_forums' ) . ' &raquo;</a></td>';
		}
		$content = $content . '</tr>';
		$content = $content . '</table>';
	}
...
function forums_output_view_topic($tmp_tid,$tmp_fid){
	...
	if (count($tmp_posts) > 0){
		...
		foreach ($tmp_posts as $tmp_post){
			...
			$content =  $content . __( 'Veröffentlicht: ', 'psource_forums' ) . date(get_option('date_format', __("j. F Y g:i", 'psource_forums' )),$tmp_post['post_stamp']);
			...
			if(current_user_can('manage_options')){
				$tmp_post_count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM " . $db_prefix . "forums_posts WHERE post_topic_ID = %d", $tmp_tid) );
				if ($tmp_post_count > 1){
					$content =  $content . '<a href="' . add_query_arg( array( 'action' => 'edit_post', 'forum_page' => $tmp_current_page, 'tid' => $tmp_tid, 'pid' => (int)$tmp_post['post_ID'] ), get_permalink() ) . '">' . __( 'Bearbeiten', 'psource_forums' ) . '</a>|<a href="?action=delete_post&page=' . $tmp_current_page . '&tid=' . $tmp_tid . '&pid=' . (int)$tmp_post['post_ID'] . '">' . __( 'Löschen', 'psource_forums' ) . '</a>';
				} else {
					$content =  $content . '<a href="' . add_query_arg( array( 'action' => 'edit_post', 'forum_page' => $tmp_current_page, 'tid' => $tmp_tid, 'pid' => (int)$tmp_post['post_ID'] ), get_permalink() ) . '">' . __( 'Bearbeiten', 'psource_forums' ) . '</a>';
				}
		...
		$content =  $content . '<table border="0" width="100%" cellpadding="0" cellspacing="0">';
		$content =  $content . '<tr>';
		$content =  $content . '<td><center>' . __( 'Keine Beiträge zum Anzeigen...', 'psource_forums' ) . '</center></td>';
		$content =  $content . '</tr>';
		$content =  $content . '</table>';
	}
...
function forums_output_new_topic($tmp_fid, $tmp_errors,$tmp_error_msg = '') {
	...
	if ($user_ID == '' || $user_ID == '0'){
		...
		$content = $content . '<table width="100%" cellspacing="2" cellpadding="5">';
		$content = $content . '<tr valign="top">';
		$content = $content . '<th scope="row">' . __( 'Titel:', 'psource_forums' ) . '</th>';
		...
		$content = $content . '</fieldset>';
		$content = $content . '<p class="submit">';
		$content = $content . '<input type="submit" name="Submit" value="' . __( 'Beitrag senden', 'psource_forums' ) . ' &raquo;" />';
		$content = $content . '</p>';
		$content = $content . '</form>';
	}
	return $content;
	...
	if ($tmp_topic_count > 0){
		if ($user_ID == '' || $user_ID == '0'){
			...
			<tr style="background-color:' . $tmp_forum_color_header . ';">
				<th ' . $style . ' ><center>' . __( 'THEMEN', 'psource_forums' ) . '</center></th>
				<th ' . $style . ' ><center>' . __( 'BEITRÄGE', 'psource_forums' ) . '</center></th>
				<th ' . $style . ' ><center>' . __( 'LETZTER POSTER', 'psource_forums' ) . '</center></th>
			</tr>';
		} else {
			$content = '<table ' . $style . ' width="100%" cellpadding="0" cellspacing="0">
			<tr style="background-color:' . $tmp_forum_color_header . ';">
				<th ' . $style . ' ><center>' . __( 'THEMEN', 'psource_forums' ) . ' (<a href="' . add_query_arg( array( 'action' => 'new_topic', 'fid' => $tmp_fid ), get_permalink() ) . '">' . __( 'Neues Thema', 'psource_forums' ) . '</a>)</center></th>
				<th ' . $style . ' ><center>' . __( 'BEITRÄGE', 'psource_forums' ) . '</center></th>
				<th ' . $style . ' ><center>' . __( 'LETZTER POSTER', 'psource_forums' ) . '</center></th>
			</tr>';
	...
	if (count($tmp_topics) > 0){
		...
		//=========================================================//
			...
			if ($tmp_topic['topic_closed'] == 1){
				$content =  $content . '<td ' . $style . ' ><center><a href="'. add_query_arg( array( 'topic' => (int)$tmp_topic['topic_ID'] ), get_permalink() ) . '">' . esc_html(stripslashes($tmp_topic['topic_title'])) . ' (' . __( 'Geschlossen', 'psource_forums' ) . ')</a></center></td>';
			} else {
		...
		if ($user_ID == '' || $user_ID == '0'){
			$content =  $content . '<table border="0" width="100%" cellpadding="0" cellspacing="0">';
			$content =  $content . '<tr>';
			$content =  $content . '<td><center>' . __( 'Keine Themen zum Anzeigen...', 'psource_forums' ) . '</center></td>';
			$content =  $content . '</tr>';
			$content =  $content . '</table>';
		} else {
			$content =  $content . '<table border="0" width="100%" cellpadding="0" cellspacing="0">';
			$content =  $content . '<tr>';
			$content =  $content . '<td><center>' . __( 'Keine Themen zum Anzeigen...', 'psource_forums' ) . '<a href="'.add_query_arg( array('action' => 'new_topic', 'fid' => $tmp_fid), get_permalink() ) .'">' . __( 'Klicke hier, um ein neues Thema zu erstellen.', 'psource_forums' ) . '</a></center></td>';
			$content =  $content . '</tr>';
			$content =  $content . '</table>';
		}
	}
...
function forums_manage_options_output() {
	...
	if(!current_user_can($perms)) {
		echo "<p>" . __( 'Netter Versuch...', 'psource_forums' ) . "</p>";  //If accessed properly, this message doesn't appear.
		return;
	}
	echo '<div class="wrap">';
	...
	if ($action == 'update_settings') {
		...
		?>
		<script type="text/javascript">
			window.location = '<?php echo $page; ?>?page=psource_forum_settings';
		</script>
		<?php
		exit();
	}
	...
	?>
	<h2><?php _e( 'Forum-Einstellungen', 'psource_forums' ) ?></h2>
	<form name="form1" method="post" action="<?php echo $page; ?>?page=psource_forum_settings&action=update_settings">
		<table class="form-table">
			<tr valign="top">
				<th scope="row"><?php _e( 'Themen pro Seite', 'psource_forums' ) ?></th>
			...
			<tr valign="top">
				<th scope="row"><?php _e( 'Beiträge pro Seite', 'psource_forums' ) ?></th>
			...
			<tr valign="top">
				<th scope="row"><?php _e( 'Maximale Anzahl an Foren', 'psource_forums' ) ?></th>
			...
			<?php if (function_exists('is_pro_site')) { ?>
				<tr valign="top">
					<th scope="row"><?php _e( 'Maximale Anzahl an Foren für aktualisierte Blogs', 'psource_forums' ) ?></th>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Upgrades zulassen', 'psource_forums' ) ?></th>
		...
		<p class="submit">
			<input type="submit" name="Submit" value="<?php _e( 'Speichern', 'psource_forums' ) ?>" />
		</p>
	</form>
	<?php
	echo '</div>';
...
function forums_manage_output() {
	...
	switch( $action ) {
		...
		$tmp_forums_count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM " . $db_prefix . "forums WHERE forum_blog_ID = %d", $wpdb->blogid) );
			...
			?>
            <h2><?php _e( 'Foren verwalten', 'psource_forums' ) ?> (<a href="admin.php?page=psource_forums&action=new_forum"><?php _e( 'Neu', 'psource_forums' ) ?></a>)</h2>
            <?php
			} else {
			?>
            <h2><?php _e( 'Foren verwalten', 'psource_forums' ) ?></h2>
            <?php
			...
			if ($forums_enable_upgrades && $tmp_forums_count == 0){
				if ( function_exists('is_pro_site') && !is_pro_site() && function_exists('psts_hide_ads') && !psts_hide_ads() ) {
					global $psts, $blog_id;
					$feature_message = __( "Rüste Deinen Blog jetzt auf, um Foren hinzuzufügen, den Speicherplatz zu erhöhen, Videos einzubetten und vieles mehr.", 'psource_forums' );
					echo '<div id="message" class="error"><p><a href="' . $psts->checkout_url($blog_id) . '">' . $feature_message . '</a></p></div>';
				} else {
				?>
					<p><a href="admin.php?page=psource_forums&action=new_forum"><?php _e( 'Klicke hier, um ein neues Forum hinzuzufügen.', 'psource_forums' ) ?></a></p>
				<?php	
				}
			} else {
			...
			if (count($tmp_forums) > 0){
				...
				echo "<td valign='top'><a href='admin.php?page=psource_forums&action=edit_forum&fid=" . (int)$tmp_forum['forum_ID'] . "' rel='permalink' class='edit'>" . __( 'Bearbeiten', 'psource_forums' ) . "</a></td>";
				echo "<td valign='top'><a href='admin.php?page=psource_forums&action=delete_forum&fid=" . (int)$tmp_forum['forum_ID'] . "' rel='permalink' class='delete'>" . __( 'Entfernen', 'psource_forums' ) . "</a></td>";
				echo "</tr>";
				$class = ('alternate' == $class) ? '' : 'alternate';
				//=========================================================//
				}
		...
		case "new_forum":
			...
			if ($tmp_forums_count < $forums_max_forums){
				?>
				<h2><?php _e( 'Neues Forum', 'psource_forums' ) ?></h2>
				<form name="form1" method="POST" action="admin.php?page=psource_forums&action=new_forum_process">
					<table class="form-table">
					<tr valign="top">
					<th scope="row"><?php _e( 'Name', 'psource_forums' ) ?></th>
					...
					<br />
					<?php _e( 'Erforderlich', 'psource_forums' ) ?></td>
					</tr>
					<tr valign="top">
					<th scope="row"><?php _e( 'Beschreibung', 'psource_forums' ) ?></th>
					...
					<br />
					<?php _e( 'Optional', 'psource_forums' ) ?></td>
					</tr>
					<tr valign="top">
					<th scope="row"><?php _e( 'Farbe 1', 'psource_forums' ) ?></th>
					...
					<br />
					<?php _e( 'Optional - z.B.: #000000 ODER #FFFFFF', 'psource_forums' ) ?></td>
					</tr>
					<tr valign="top">
					<th scope="row"><?php _e( 'Farbe 2', 'psource_forums' ) ?></th>
					...
					<tr valign="top">
					<th scope="row"><?php _e( 'Kopfzeilenfarbe', 'psource_forums' ) ?></th>
					...
					<tr valign="top">
					<th scope="row"><?php _e( 'Rahmenfarbe', 'psource_forums' ) ?></th>
					...
					<tr valign="top">
					<th scope="row"><?php _e( 'Rahmengrösse', 'psource_forums' ) ?></th>
		...
		case "new_forum_process":
			if ($_POST['forum_name'] == ''){
					?>
						<h2><?php _e( 'Neues Forum', 'psource_forums' ) ?></h2>
                        <p><?php _e( 'Bitte fülle alle geforderten Felder aus', 'psource_forums' ) ?></p>
						<form name="form1" method="POST" action="admin.php?page=psource_forums&action=new_forum_process">
							<table class="form-table">
							<tr valign="top">
							<th scope="row"><?php _e( 'Name', 'psource_forums' ) ?></th>
				...
				echo "
				<script>
				window.location='admin.php?page=psource_forums&updated=true&updatedmsg=" . urlencode( __( 'Forum hinzugefügt.', 'psource_forums' ) ) . "';
				</script>
				";
				exit();
			}
		...
		?>
			<h2><?php _e( 'Forum bearbeiten', 'psource_forums' ) ?></h2>
            <form name="form1" method="POST" action="admin.php?page=psource_forums&action=edit_forum_process">
		...
		<br />
            ...
            <p class="submit">
            <input type="submit" name="Submit" value="<?php _e( 'Änderungen speichern', 'psource_forums' ) ?>" />
            </p>
            </form>
        <?php
		break;
		...
		case "edit_forum_process":
			...
			if ($tmp_forum_count > 0){
				if ($_POST['forum_name'] == ''){
					?>
						<h2><?php _e( 'Forum bearbeiten', 'psource_forums' ) ?></h2>
                        <p><?php _e( 'Bitte fülle alle erforderlichen Felder aus', 'psource_forums' ) ?></p>
						<form name="form1" method="POST" action="admin.php?page=psource_forums&action=edit_forum_process">
			    ...
			    <br />
						...
						<p class="submit">
						<input type="submit" name="Submit" value="<?php _e( 'Änderungen speicherns', 'psource_forums' ) ?>" />
						</p>
						</form>
					<?php
				} else {
					...
					echo "
					<script>
					window.location='admin.php?page=psource_forums&updated=true&updatedmsg=" . urlencode( __( 'Einstellungen gespeichert.', 'psource_forums' ) ) . "';
					</script>
					";
					exit();
				}
		...
		case "delete_forum":
		?>
				<h2><?php _e( 'Forum entfernen', 'psource_forums' ) ?></h2>
                <p><?php _e( 'Möchtest Du dieses Forum wirklich entfernen? Alle Themen und Beiträge werden gelöscht.', 'psource_forums' ) ?></p>
				<form name="step_one" method="POST" action="admin.php?page=psource_forums&action=delete_forum_process">
				<input type="hidden" name="fid" value="<?php echo (int)$_GET['fid']; ?>" />
				<p class="submit">
				<input type="submit" name="Submit" value="<?php _e( 'Fortfahren', 'psource_forums' ) ?>" />
				<input type="submit" name="Cancel" value="<?php _e( 'Abbrechen', 'psource_forums' ) ?>" />
				</p>
				</form>
        <?php
		break;
		...
		case "delete_forum_process":
			if ( isset($_POST['Cancel']) ) {
				...
				echo "
				<script>
				window.location='admin.php?page=psource_forums&updated=true&updatedmsg=" . urlencode( __( 'Forum entfernt.', 'psource_forums' ) ) . "';
				</script>
				";
			}
			exit();
